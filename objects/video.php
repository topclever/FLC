...
if (!class_exists('Video')) {
    ...
    class Video {
        ...
        private $isSuggested;
        private $isStandardFeatured;
        private $isPremiumFeatured;
        private $inputLinkTitle;
        private $inputLinkAddress;
...
// check if category exists
            ...
            if (!empty($this->id)) {
                ...
                $sql = "UPDATE videos SET title = '{$this->title}',clean_title = '{$this->clean_title}',"
                        ...
                        . " encoderURL = '{$this->encoderURL}', filepath = '{$this->filepath}' , filesize = '{$this->filesize}', isStandardFeatured = '{$this->isStandardFeatured}', isPremiumFeatured = '{$this->isPremiumFeatured}', inputLinkTitle = '{$this->inputLinkTitle}', inputLinkAddress = '{$this->inputLinkAddress}' , modified = now()"
                        . " WHERE id = {$this->id}";
            } else {
...
// i would like to simplify the big part of the method above in this method, but won't work as i want.
        ...
        function getIsStandardFeatured() {
            return $this->isStandardFeatured;
        }

        function getIsPremiumFeatured() {
            return $this->isPremiumFeatured;
        }
        ...
        // tempcode featured video set
        function setIsStandardFeatured($isStandardFeatured) {
            if (empty($isStandardFeatured) || $isStandardFeatured === "false") {
                $this->isStandardFeatured = 0;
            } else {
                $this->isStandardFeatured = 1;
                $sql = "UPDATE videos SET isStandardFeatured = 0 WHERE isStandardFeatured = 1 ";
                $res = sqlDAL::writeSql($sql);
            }
        }

        // tempcode featured video set
        function setIsPremiumFeatured($isPremiumFeatured) {
            if (empty($isPremiumFeatured) || $isPremiumFeatured === "false") {
                $this->isPremiumFeatured = 0;
            } else {
                $this->isPremiumFeatured = 1;

                $sql = "UPDATE videos SET isPremiumFeatured = 0 WHERE isPremiumFeatured = 1 ";
                $res = sqlDAL::writeSql($sql);
            }
        }

        // tempcode sources htmllink
        function setSourcesLink($sourcesLinkArray, $user_id, $video_id) {
            $sql = "DELETE FROM sources WHERE user_id = '{$user_id}' AND video_id = '{$video_id}' ";
            $resp = sqlDAL::writeSql($sql);

            foreach ($sourcesLinkArray as $value) {
                $sql = "INSERT INTO sources "
                        . "(inputLinkTitle, inputLinkAddress, user_id, video_id) values "
                        . "('{$value->inputLinkTitle}', '{$value->inputLinkAddress}', '{$user_id}', '{$video_id}')";
                $insert_row = sqlDAL::writeSql($sql);
                if(!$insert_row)
                    return false;
            }
        ...
        // function setInputLinkTitle($inputLinkTitle) {
        //     $this->inputLinkTitle = $inputLinkTitle;
        // }

        // function setInputLinkAddress($inputLinkAddress) {
        //     $this->inputLinkAddress = $inputLinkAddress;
        // }
...
// check if the video is not public 
        ...
        // tempcode featured video 
        static function getStandardFeaturedVideo($id = "", $status = "viewable", $ignoreGroup = false, $random = false, $suggestedOnly = false, $showUnlisted = false, $ignoreTags = false, $activeUsersOnly = true) {
            global $global, $config;
            if ($config->currentVersionLowerThen('5')) {
                return false;
            }
            ...
            // tempcode featured video set
            $sql .= " FROM (SELECT * FROM videos WHERE isStandardFeatured = 1 LIMIT 1) as v "
                    . "LEFT JOIN categories c ON categories_id = c.id "
                    . "LEFT JOIN users u ON v.users_id = u.id "
                    . "LEFT JOIN (SELECT * FROM videos WHERE isStandardFeatured = 1 LIMIT 1) nv ON v.next_videos_id = nv.id "
                    . " WHERE 1=1 ";
            // if ($activeUsersOnly) {
            //     $sql .= " AND u.status = 'a' ";
            // }
        ...
        // tempcode featured video 
        static function getPremiumFeaturedVideo($id = "", $status = "viewable", $ignoreGroup = false, $random = false, $suggestedOnly = false, $showUnlisted = false, $ignoreTags = false, $activeUsersOnly = true) {
            ...
            // tempcode featured video set
            $sql .= " FROM (SELECT * FROM videos WHERE isPremiumFeatured = 1 LIMIT 1) as v "
                    . "LEFT JOIN categories c ON categories_id = c.id "
                    . "LEFT JOIN users u ON v.users_id = u.id "
                    . "LEFT JOIN (SELECT * FROM videos WHERE isPremiumFeatured = 1 LIMIT 1) nv ON v.next_videos_id = nv.id "
                    . " WHERE 1=1 ";
            // if ($activeUsersOnly) {
            //     $sql .= " AND u.status = 'a' ";
            // }
        ...
        // tempcode featured video most watched video
        static function getMostWatchedVideo($id = "", $status = "viewable", $ignoreGroup = false, $random = false, $suggestedOnly = false, $showUnlisted = false, $ignoreTags = false, $activeUsersOnly = true) {
            global $global, $config;
            if ($config->currentVersionLowerThen('5')) {
                return false;
            }
            ...
            // tempcode featured video set
            $sql .= " FROM (SELECT * FROM videos ORDER BY views_count DESC LIMIT 1) as v "
                    . "LEFT JOIN categories c ON categories_id = c.id "
                    . "LEFT JOIN users u ON v.users_id = u.id "
                    . "LEFT JOIN (SELECT * FROM videos ORDER BY views_count DESC LIMIT 1) nv ON v.next_videos_id = nv.id "
                    . " WHERE 1=1 ";
            // if ($activeUsersOnly) {
            //     $sql .= " AND u.status = 'a' ";
            // }
        ...
        // tempcode get sources list
        static function getSourcesList($user_id, $video_id) {
            $sql = "SELECT * FROM `sources` WHERE user_id = '{$user_id}' AND video_id='{$video_id}' ";
            ...
            if($fullResult) {
                foreach ($fullResult as $row) {
                    ...
                    $editBtn = '<button type="button" style="float:left" class="btn btn-xs btn-default command-edit" id="edit_button'.$table_id.'" data-toggle="tooltip" title="Edit Row" data-placement="left" onclick="edit_row('.$table_id.')"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button>';
                    $deleteBtn = '<button type="button" style="float:left" class="btn btn-xs btn-default command-edit" id="delete_button'.$table_id.'" data-toggle="tooltip" title="Delete Row" data-placement="left" onclick="delete_row('.$table_id.')"><span class="glyphicon glyphicon-erase" aria-hidden="true"></span></button>';
                    $saveBtn = '<button type="button" style="display:none; float:left" class="btn btn-xs btn-default command-edit" id="save_button'.$table_id.'" data-toggle="tooltip" title="Save Row" data-placement="left" onclick="save_row('.$table_id.')"><span class="glyphicon glyphicon-save" aria-hidden="true"></span></button>';

                    $return .= "<tr id='row".$table_id."'>";
                    $return .= "<td id='inputLinkTitle_row".$table_id."'>".$row['inputLinkTitle']."</td>";
                    $return .= "<td id='inputLinkAddress_row".$table_id."'>".$row['inputLinkAddress']."</td>";
                    $return .= "<td>".$editBtn.$saveBtn.$deleteBtn."</td></tr>";
                }
        ...
        // tempcode get sources list
        static function getSourcesArray($user_id, $video_id) {
            $sql = "SELECT * FROM `sources` WHERE user_id = '{$user_id}' AND video_id='{$video_id}' ";
