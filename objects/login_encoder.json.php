...
if (!empty($_GET['encodedPass'])) {
    ...
    // $object->error = $_POST['user'].$_POST['pass'];
...
$user = new User(0, null, $_POST['pass'], $_POST['user']);
$resp = $user->login_encoder(false, @$_POST['encodedPass']);
TimeLogEnd($timeLog, __LINE__);
...
if(!empty($_POST['redirectUri']))
    $object->redirectUri = $global['webSiteRootURL']."user";
else
    $object->redirectUri = $global['webSiteRootURL'];
