...
class User {
    ...
    //     }
    // }
    // tempcode forgot password. add email params getUserFromEmail
    function __construct($id, $user = "", $password = "", $email = "") {
        if (empty($id)) {
            // get the user data from user and pass
            if($user) {
                $this->user = $user;
                ...
                _error_log("aVideoEncoder.json: Create User: " . json_encode($this));
            }
        } else {
            // get data from id
            $this->load($id);
    ...
    // tempcode forgot password
    private function loadFromEmail($email) {
    ...
    // tempcode encoder login
    function login_encoder($noPass = false, $encodedPass = false) {
        ...
        if ($noPass) {
            $user = $this->find_encoder($this->email, false, true);
        } else {
            $user = $this->find_encoder($this->email, $this->password, true, $encodedPass);
        }

        // disabled origin captcha for google captcha
        ...
        if (!empty($user) && empty($user['isAdmin']) && empty($user['emailVerified']) && !empty($advancedCustomUser->unverifiedEmailsCanNOTLogin)) {
            ...
            if (empty($_COOKIE['user']) || empty(empty($_COOKIE['pass']))) {
                ...
                setcookie("email", $user['email'], $cookie, "/", $_SERVER['HTTP_HOST']);
    ...
    // tempcode featured video isPremiumUser
    static function isPremiumUser($user_id) { 
        global $global;
        $sql = "SELECT * FROM (SELECT users_id, STATUS FROM subscriptions WHERE STATUS = 'a') AS s 
            LEFT JOIN (SELECT id FROM users WHERE users.id = '".$user_id."') AS u 
            ON u.id = s.users_id 
            LIMIT 1";

        $user = false;
        $res = sqlDAL::readSql($sql . ";");
        $parray = sqlDAL::fetchAllAssoc($res);
        sqlDAL::close($res);
        if ($res != false) {
            if(count($parray) == 1)
                $user = true;
...
";
    ...
    private function find($user, $pass, $mustBeactive = false, $encodedPass = false) {
        ...
        // tempcode removed user name login
        $sql = "SELECT * FROM users WHERE email = ? ";
        // $sql = "SELECT * FROM users WHERE user = ? OR email = '".$user."' ";
    ...
    // tempcode findencoder
    private function find_encoder($user, $pass, $mustBeactive = false, $encodedPass = false) {
        global $global, $advancedCustom;
        $formats = "";
        $values = array();
        $user = $global['mysqli']->real_escape_string($user);
