...
class Subscribe {
    ...
    static function getButton($user_id) {
        ...
        // tempcode subscribe button
        $subscribe = "<div class=\"btn-group\" >"
                . "<button class='btn btn-xs subsB subs{$user_id} subscribeButton{$user_id}'  style='padding:7px' "
                ...
                . "<button class='btn btn-xs subsB subs{$user_id}' style='padding:7px'><b class='textTotal{$user_id}'>{$total}</b></button> "
                . "</div>";
        ...
        if (User::isLogged()) {
            ...
            if(!empty($subs) && $subs['status']==='a'){
                $subscribe .= "<button class='btn btn-xs subsB subscribeButton{$user_id} subscribed subs{$user_id}'  style='padding:7px'><i class='fas fa-play-circle'></i> <b class='text'>" . __("Subscribed") . "</b></button>";
                $subscribe .= "<button class='btn btn-xs subsB subscribed subs{$user_id}'  style='padding:7px'><b class='textTotal{$user_id}'>$total</b></button>";
            }else{
                $subscribe .= "<button class='btn btn-xs subsB subscribeButton{$user_id} subs{$user_id}' style='padding:7px'><i class='fas fa-play-circle'></i> <b class='text'>" . __("Subscribe") . "</b></button>";
                    $subscribe .= "<button class='btn btn-xs subsB subs{$user_id}' style='padding:7px'><b class='textTotal{$user_id}'>$total</b></button>";
            }
            $subscribe .= "</div>";
            ...
            $subscribe .= '<span class=" notify' . $user_id . ' ' . $notify . '"><button onclick="toogleNotify' . $user_id . '();" class="btn btn-default btn-xs " style="padding:7px" data-toggle="tooltip" 
                                   title="' . __("Stop getting notified for every new video") . '">
                                <i class="fa fa-bell" ></i>
                            </button></span><span class=" notNotify' . $user_id . ' ' . $notNotify . '"><button onclick="toogleNotify' . $user_id . '();" class="btn btn-default btn-xs "  style="padding:7px" data-toggle="tooltip" 
