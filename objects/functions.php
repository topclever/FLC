...
// guid
    ...
    // tempcode download mp3
    function getVideosURLMP3($fileName) {
        global $global;
        if (empty($fileName)) {
            return array();
        }
        ...
        $files = array_merge($files, _getImagesURL($fileName, 'mp3'));
        $time = microtime();
        $time = explode(' ', $time);
        $time = $time[1] + $time[0];
        $finish = $time;
    ...
    // tempcode get sources URL
    function getSourcesPDF($fileName, $cache = true) {
        global $global;
        if (empty($fileName)) {
            return array();
        }
    ...
    function getSourcesURL($video_id, $user_id) {
