        ...
        <div class="row">
            <div class="col-md-12 watch8-action-buttons text-muted">
                <?php if (empty($advancedCustom->disableShareAndPlaylist)) { ?>
                    ...
                    // get sources url
                    $pdfUrl = getSourcesPDF($video['filename'])['pdf'];
                    $sourcesList = getSourcesURL($video['id'], User::getId());
                    // print_r($pdfUrl);

                    if($pdfUrl || $sourcesList) {
                    ?>
                    <!-- tempcode add Sources button on video page -->
                    <a href="#" class="btn btn-default no-outline" id="sourcesBtn">
...
<!-- tempcode sources list -->
    <div class="row bgWhite list-group-item menusDiv" id="sourcesDiv">
        <div class="tabbable-panel">
            <div class="list-group">
                <?php
                    if($pdfUrl) {
                    ?> 
                    <a href="<?php echo $pdfUrl['url']; ?>" class="list-group-item list-group-item-action" download>
                        <i class="fas fa-download"></i> <?php echo $pdfUrl['filename']; ?>
                    </a>
                <?php
                    }
                    if($sourcesList) {
                        foreach ($sourcesList as $key => $value) {
                            ?>
                            <a href="<?php echo $value['inputLinkAddress']; ?>" class="list-group-item list-group-item-action" target="_blank">
                                <i class="fas fa-book"></i> <?php echo $value['inputLinkTitle']; ?>
                            </a>
                        <?php
                        }
                    }
...
<?php if (!empty($filesToDownload) && CustomizeUser::canDownloadVideosFromVideo($video['id'])) { ?>
    <div class="row bgWhite list-group-item menusDiv" id="downloadDiv">
        <div class="tabbable-panel">
            <div class="list-group">
                <?php
                // tempcode download mp3, mp4
                foreach ($filesToDownload as $theLink) {
                    ?> 
                    <a href="<?php echo $theLink['url']; ?>" class="list-group-item list-group-item-action" download>
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            if ($video['type']!=='notfound' && $video['type'] !== 'article') {
                ...
                <div class="col-xs-8 col-sm-10 col-lg-10" itemprop="description">
                    <?php 
                    // tempcode
                    echo "<div>".substr($video['description'], 0, strpos($video['description'], "</p>")); 
                    echo ("<span id='dots'>...</span>");
                    echo ("<div id='more'>");
                    echo substr($video['description'], strpos($video['description'], "</p>")); 
                    echo("</div></div>");
                    ...
                    ?>
                    <a onclick="myFunction()" id="myBtn" style="cursor: pointer;">Read more</a>
                </div>
                <?php
            }
            ?>
...
<script>
    $(document).ready(function () {
        ...
        // tempcode download sources
        $("#sourcesDiv").slideUp();
        $("#sourcesBtn").click(function () {
            $(".menusDiv").not("#sourcesDiv").slideUp();
