                    ...
                    if (User::canUpload()) {
                        if (empty($advancedCustom->doNotShowEncoderButton)) {
                            ...
                            <form id="formEncoderVideosM" method="post" action="<?php echo $config->getEncoderURL(); ?>" target="encoder">
                                <!-- tempcode encoder login send the url, email, password to encoder site -->
                                <input type="hidden" name="webSiteRootURL" value="<?php echo $global['webSiteRootURL']; ?>" />
                                <input type="hidden" name="user" value="<?php echo User::getMail(); ?>" />
                ...
                <div class="modal-body" style="max-height: 70vh; overflow-y: scroll;">
                    <div id="postersImage">
                        <ul class="nav nav-tabs">
                            ...
                            <li><a data-toggle="tab" href="#pmp3">Mp3</a></li>
                            <li><a data-toggle="tab" href="#psources">Sources</a></li>
                        ...
                        <div class="tab-content">
                            ...
                            <!-- tempcode upload mp3 -->
                            <div id="pmp3" class="tab-pane fade"> 
                                <input id="input-mp3" type="file" class="file-loading" accept="audio/mp3">
                                <div class="kv-fileinput-error file-error-message" style="color:#2286f9;background-color: #dee9f2;">
                                    <ul>
                                        <li data-file-id="preview-1599157590220_17-0">File Maximum allowed upload size is 300MB.
                                    </li>
                                    </ul>
                                </div>
                            </div>

                            <!-- tempcode upload sources -->
                            <div id="psources" class="tab-pane fade">
                                <ul class="nav nav-tabs">
                                    <li class="active"><a data-toggle="tab" href="#pdf">PDF</a></li>
                                    <li><a data-toggle="tab" href="#hlink">HTML Link</a></li> 
                                </ul>

                                <div class="tab-content">
                                    <div id="pdf" class="tab-pane fade in active">
                                        <input id="input-pdf" type="file" class="file-loading">
                                    </div>
                                    <div id="hlink" class="tab-pane fade">
                                        <form class="form-compact"  id="htmlLinkForm" onsubmit="">
                                            <div class="titles">
                                                <table class="table table-condensed table-hover table-striped" id="data_table">
                                                    <thead>
                                                        <tr>
                                                            <th><?php echo __("HTML Link Title"); ?></th>
                                                            <th><?php echo __("HTML Link Address"); ?></th>
                                                ...
                                                <input type="hidden" id="sourcesArray">

                                                <input type="text" id="inputLinkTitle" class="form-control" placeholder="<?php echo __("HTML Link Title"); ?>" required>
                                                <input type="text" id="inputLinkAddress" class="form-control" style="margin-top: 5px;" placeholder="<?php echo __("HTML Link Address"); ?>" required>

                                                <button type="button" class="btn btn-default" style="float: right;" onclick="add_row();">
                                                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span> <?php echo __("Add Html Link"); ?>
                                                </button>
                            ...
                            <div id="pimages" class="tab-pane fade">
                                ...
                                <div class="tab-content">
                                    <div id="jpg" class="tab-pane fade in active">
                                        <input id="input-jpg" type="file" class="file-loading" accept="image/jpg, jpeg, png">
                                    </div>
                                    <div id="pjpg" class="tab-pane fade">
                                        <input id="input-pjpg" type="file" class="file-loading" accept="image/jpg, jpeg, png">
                                    </div>
...
<script>
    ...
    function saveFeaturedVideo(videos_id, add, type) {
        modal.showPleaseWait();
        $.ajax({
            url: '<?php echo $global['webSiteRootURL']; ?>objects/videoFeaturedSave.json.php',
            method: 'POST',
            data: {
                'videos_id': videos_id,
                'add': add,
...
?>
    ...
    function editVideo(row) {
        ...
        // $('#inputLinkTitle').val(row.inputLinkTitle);
        // $('#inputLinkAddress').val(row.inputLinkAddress);
...
?>
        ...
        reloadFileInput(row);
        $('#input-mp3, #input-pdf, #input-jpg, #input-gif,#input-pjpg, #input-pgif, #input-webp').on('fileuploaded', function (event, data, previewId, index) {
            $("#grid").bootgrid("reload");
        })

        // tempcode source list load
        // console.log('videos_id========>', row.id);
        $.ajax({
        url: '<?php echo $global['webSiteRootURL']; ?>objects/getSourcesList.json.php',
                data: {
                ...
                success: function (response) { 
                    ...
                    // console.log('response', response.sarray, response.rows);
                    document.getElementById("data_table_body").innerHTML=response.rows; 
                    $('#sourcesArray').val(response.sarray);
                }
        });
        /////////////////////////
        waitToSubmit = true;
    ...
    function reloadFileInput(row) {
        ...
        $('#input-mp3, #input-pdf, #input-jpg, #input-gif, #input-pjpg, #input-pgif, #input-webp').fileinput('destroy');

        // tempcode upload mp3
        $("#input-mp3").fileinput({
            uploadUrl: "<?php echo $global['webSiteRootURL']; ?>objects/uploadMp3.php?video_id=" + row.id + "&type=mp3",
            autoReplace: true,
            overwriteInitial: true,
            showUploadedThumbs: false,
            maxFileCount: 1, 
            maxFileSize: 307200,
            initialCaption: row.clean_title + '.mp3',
            ...
            console.log(data, 'File Upload Error', 'ID: ' + data.fileId + ', Thumb ID: ' + data.previewId);
        ...
        // tempcode upload pdf
        $("#input-pdf").fileinput({
            uploadUrl: "<?php echo $global['webSiteRootURL']; ?>objects/uploadPdf.php?video_id=" + row.id + "&type=pdf",
            autoReplace: true,
            overwriteInitial: true,
            showUploadedThumbs: false,
            maxFileCount: 1, 
            maxFileSize: 102400,
            initialCaption: row.clean_title + '.pdf',
            initialPreviewShowDelete: false,
            showRemove: false,
            showClose: false,
            layoutTemplates: {actionDelete: ''}, // disable thumbnail deletion
    ...
    function saveVideo(closeModal) {
        ...
        // tempcode send sourcearray
        var sourcesArray = $('#sourcesArray').val();
        console.log('ID',$('#inputVideoId').val(), "userid", $('#inputUserOwner_id').val(), sourcesArray);
        ...
        url: '<?php echo $global['webSiteRootURL']; ?>objects/videoAddNew.json.php',
            ...
            "categories_id": $('#inputCategory').val(),
                ...
                type: 'post',
                success: function (response) {
                    // tempcode sources htmllink check the parse is needed or not
                    var response = JSON.parse(response);
                    console.log(response);
                    if (response.status === "1" || response.status === true || response.status === 'true') {
...
?>
        ...
        $('#videoIsAd').trigger("change");
        reloadFileInput();
        $('#input-mp3, #input-pdf, #input-jpg, #input-gif,#input-pjpg, #input-pgif').on('fileuploaded', function (event, data, previewId, index) {
            $("#grid").bootgrid("reload");
        });
        videos_id = 0;
    }
...
<?php } ?>
            ...
            $('#videoIsAd').prop('checked', false);
            $('#videoIsAd').trigger("change");
            $('#input-mp3, #input-pdf, #input-jpg, #input-gif, #input-pjpg, #input-pgif').fileinput('destroy');
...
?>
                    ...
                    for (var k in row.videosURL) {
                        ...
                        // tempcode download
                        download += '<a href="' + url + '" class="btn btn-default btn-xs" download ><span class="fa fa-download " aria-hidden="true"></span> ' + k + '</a><br>';
                    }
...
?>
            ...
            $('.videoPlaylist').each(function (i, obj) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        // tempcode featured video set
                        $.ajax({
                            url: '<?php echo $global['webSiteRootURL']; ?>objects/videoFeatured.json.php',
                            data: {"videos_id": videos_id},
                            type: 'post',
                            success: function (response) {
                                 // console.log(response, response.isStandardFeatured, response.isPremiumFeatured);
                                 var res = JSON.parse(response);
                                // var lists = "";
                                lists += '<div class="material-small material-switch"><input onchange="saveFeaturedVideo(' + videos_id + ', $(this).is(\':checked\'), ' + '\'standard\'' + ')" data-toggle="toggle" type="checkbox" id="standardVideo' + videos_id + "_" + videos_id + '" value="1" ' + (res.isStandardFeatured ? "checked" : "") + ' videos_id="' + videos_id + '" ><label for="standardVideo' + videos_id + "_" + videos_id + '" class="label-primary"></label>  ' + '<?php echo __('Standard Video featured'); ?>' + '</div>';

                                lists += '<div class="material-small material-switch"><input onchange="saveFeaturedVideo(' + videos_id + ', $(this).is(\':checked\'), ' + '\'premium\'' + ')" data-toggle="toggle" type="checkbox" id="premiumVideo' + videos_id + "_" + videos_id + '" value="1" ' + (res.isPremiumFeatured ? "checked" : "") + ' videos_id="' + videos_id + '" ><label for="premiumVideo' + videos_id + "_" + videos_id + '" class="label-primary"></label>  ' + '<?php echo __('Premium Video featured'); ?>' + '</div>';
            ...
            .end().find(".command-inactive").on("click", function (e) {
                        // tempcode 4.correct notification activate video and send notification
                var row_index = $(this).closest('tr').index();
                var row = $("#grid").bootgrid("getCurrentRows")[row_index];
                modal.showPleaseWait();
                $.ajax({
...
?>
    ...
    // tempcode sources management-----------------------------------
    function edit_row(no)
    {
        document.getElementById("edit_button"+no).style.display="none";
        document.getElementById("save_button"+no).style.display="block";

        var inputLinkTitle=document.getElementById("inputLinkTitle_row"+no);
        var inputLinkAddress=document.getElementById("inputLinkAddress_row"+no); 

        var inputLinkTitle_data=inputLinkTitle.innerHTML;
        var inputLinkAddress_data=inputLinkAddress.innerHTML; 

        inputLinkTitle.innerHTML="<input style='font-size:14px; height:25px;' class='form-control' type='text' id='inputLinkTitle_text"+no+"' value='"+inputLinkTitle_data+"'>";
        inputLinkAddress.innerHTML="<input style='font-size:14px; height:25px;' class='form-control' type='text' id='inputLinkAddress_text"+no+"' value='"+inputLinkAddress_data+"'>"; 
    }

    function save_row(no)
    {
        var inputLinkTitle=document.getElementById("inputLinkTitle_text"+no).value;
        var inputLinkAddress=document.getElementById("inputLinkAddress_text"+no).value; 
        if(inputLinkTitle == '' || inputLinkAddress == '')
            return;

        document.getElementById("inputLinkTitle_row"+no).innerHTML=inputLinkTitle;
        document.getElementById("inputLinkAddress_row"+no).innerHTML=inputLinkAddress; 

        document.getElementById("edit_button"+no).style.display="block";
        document.getElementById("save_button"+no).style.display="none";

        var sourcesArray = JSON.parse($('#sourcesArray').val());
        for (var i = 0; i < sourcesArray.length; i++) {
          if (sourcesArray[i].id === no) {
            sourcesArray[i].inputLinkTitle = inputLinkTitle;
            sourcesArray[i].inputLinkAddress = inputLinkAddress;
            break;
          }
        }
        $('#sourcesArray').val(JSON.stringify(sourcesArray));

        // console.log($('#sourcesArray').val());
    }
    ...
    function delete_row(no)
    {
        ...
        var sourcesArray = JSON.parse($('#sourcesArray').val());
        sourcesArray = sourcesArray.filter(function(el){ return el.id != no; });
        $('#sourcesArray').val(JSON.stringify(sourcesArray));
    ...
    function add_row()
    {
        var inputLinkTitle=document.getElementById("inputLinkTitle").value;
        var inputLinkAddress=document.getElementById("inputLinkAddress").value; 
        if(inputLinkTitle == '' || inputLinkAddress == '')
            return;

        var table=document.getElementById("data_table");
        var table_len=(table.rows.length);
        var table_id = Math.floor((new Date()).getTime() / 1000);

        var editBtn = '<button type="button" style="float:left" class="btn btn-xs btn-default command-edit" id="edit_button' + table_id + '" data-toggle="tooltip" title="Edit Row" data-placement="left" onclick="edit_row('+table_id+')"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></button>';
        var deleteBtn = '<button type="button" style="float:left" class="btn btn-xs btn-default command-edit" id="delete_button' + table_id + '" data-toggle="tooltip" title="Delete Row" data-placement="left" onclick="delete_row('+table_id+')"><span class="glyphicon glyphicon-erase" aria-hidden="true"></span></button>';
        var saveBtn = '<button type="button" style="display:none; float:left" class="btn btn-xs btn-default command-edit" id="save_button' + table_id + '" data-toggle="tooltip" title="Save Row" data-placement="left" onclick="save_row('+table_id+')"><span class="glyphicon glyphicon-save" aria-hidden="true"></span></button>';


        var row = table.insertRow(table_len).outerHTML="<tr id='row"+table_id+"'>"+
        "<td id='inputLinkTitle_row"+table_id+"'>"+inputLinkTitle+"</td>"+
        "<td id='inputLinkAddress_row"+table_id+"'>"+inputLinkAddress+"</td>"+
        "<td>"+editBtn+saveBtn+deleteBtn+"</td></tr>"; 

        document.getElementById("inputLinkTitle").value="";
        document.getElementById("inputLinkAddress").value=""; 

        var obj = '{"id": ' + table_id + ', "inputLinkTitle": "' + 
            inputLinkTitle + '", "inputLinkAddress": "' + inputLinkAddress +'"}';

        var sourcesArray = JSON.parse($('#sourcesArray').val());
        sourcesArray.push(JSON.parse(obj)); 
        $('#sourcesArray').val(JSON.stringify(sourcesArray));
