...
$cmd = "rm {$removefile}";
$log->add("Client Check remove backup files: {$removefile}");
exec($cmd . " 2>&1", $output, $return_val);
if ($return_val !== 0) {
    $log->add("Client remove backup files Error: " . print_r($output, true));
}
$log->add("Client Check remove backup files: Nice!");
//--------------------------------------------------
