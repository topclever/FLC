...
function createGallerySection($videos, $crc = "", $get = array(), $ignoreAds = false) {
    ...
    foreach ($videos as $value) {
        ...
        // echo "getNameIdentificationById----".User::getNameIdentificationById($value['users_id']);
        // echo "getEmailVerifiedIcon----".User::getEmailVerifiedIcon($value['users_id']);
        // tempcode adjustment
                    ...
                    <?php if (empty($_GET['catName'])) { ?>
                        <a class="label label-default" href="<?php echo $global['webSiteRootURL']; ?>cat/<?php echo $value['clean_category']; ?>">
                            ...
                            if (!empty($value['iconClass'])) {
                                ?>
                                <image src="<?php echo $value['iconClass']; ?>" width=12 height=12 /> 
                                <?php
                            }
                            ?>
                            <!-- tempcode category image -->
                            <span style="margin-top: 8px"><?php echo $value['category']; ?></span>
                        </a>
                    <?php } ?>
                    <?php
                    @$timesG[__LINE__] += microtime(true) - $startG;
                    ...
                    <div style="position: relative; overflow: visible; z-index: 3;" class="dropup">
                        ...
                        <ul class="dropdown-menu dropdown-menu-left" role="menu">
                            ...
                            foreach ($files as $key => $theLink) {
                                ...
                                ?>
                                <li>
                                    <a href="<?php echo $theLink['url']; ?>?download=1&title=<?php echo urlencode($value['title'] . "_{$key}_.{$path_parts['extension']}"); ?>" download>
